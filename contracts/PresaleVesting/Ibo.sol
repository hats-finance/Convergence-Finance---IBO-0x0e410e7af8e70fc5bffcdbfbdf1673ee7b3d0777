...
contract Ibo is ERC721Enumerable, Ownable2Step {
    ...
    constructor(
        ...
        iboStartTimestamp = block.timestamp;
    ...
    function deposit(
        ...
        uint256 _totalCvgDue = totalCvgDuePerBond[bondId];
        {
            ...
            /// @dev peprivilege
            if (block.timestamp < _iboStartTimestamp + 69 * 1 minutes) {
            ...
            depositedUsdValue =
                amountIn *
                cvgOracle.getAndVerifyOracleAndAggregatorPrices(_bondParams.token) *
                10 ** (18 - _bondParams.token.decimals());
    ...
    function getBondView(uint256 bondId) external view returns (BondView memory bondView) {
        ...
        (
            ...
            bool verify3,
            bool verify4
        ) = cvgOracle.getOracleAndAggregatorPrices(IERC20Metadata(_bondParams.token));

        {
            bondPriceUsd = _computeCvgBondUsdPrice(CVG_PRICE_NO_ROI, _bondParams, iboStartTimestamp, alreadySold);
        }

        bondView = BondView({
            bondAddress: address(this),
            bondRoi: uint40(_depositRoi(bondId)),
            ...
            isValid: verify1 && verify2 && verify3 && verify4
        });
    }
