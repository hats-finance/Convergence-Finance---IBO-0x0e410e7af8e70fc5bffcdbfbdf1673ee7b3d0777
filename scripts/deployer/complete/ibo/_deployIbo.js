...
async function main() {
  ...
  const crvContract = await ethers.getContractAt(
    TestHelper.erc20ArtifactName,
    TOKENS.CRV.address
  );
  ...
  if (!h.getTrigger(DEPLOY_ORACLE_CONTRACT)) {
    const convergencePriceOracleContract = await hre.ethers.deployContract(
      ORACLE_CONTRACT,
      [treasuryDao]
    );
    ...
    h.writeFile(
      ORACLE_CONTRACT,
      await convergencePriceOracleContract.getAddress(),
  ...
  if (!h.getTrigger(SET_FRAX_ORACLE_PARAMETERS)) {
    ...
    await (
      await convergencePriceOracleContract
        .connect(treasuryDao)
        .setTokenOracleParams(frax.address, {
          ...
          deltaAggregatorCvgOracle:
            frax.bond_config.oracleParams.deltaAggregatorCvgOracle, // 10% delta error allowed
          twapInterval: 0,
  ...
  if (!h.getTrigger(SET_CRV_ORACLE_PARAMETERS)) {
    ...
    await (
      await convergencePriceOracleContract
        .connect(treasuryDao)
        .setTokenOracleParams(crv.address, {
          ...
          deltaAggregatorCvgOracle:
            crv.bond_config.oracleParams.deltaAggregatorCvgOracle, // 10% delta error allowed
          twapInterval: 0,
          maxLastUpdateAggregator: 800_600_400,
        })
    ).wait();
  ...
  if (!h.getTrigger(SET_WETH_ORACLE_PARAMETERS)) {
    ...
    await (
      await convergencePriceOracleContract
        .connect(treasuryDao)
        .setTokenOracleParams(weth.address, {
          ...
          deltaAggregatorCvgOracle:
            weth.bond_config.oracleParams.deltaAggregatorCvgOracle, // 10% delta error allowed
          twapInterval: 0,
          maxLastUpdateAggregator: 800_600_400,
        })
    ).wait();
  ...
  if (!h.getTrigger(SET_CVX_ORACLE_PARAMETERS)) {
    ...
    await (
      await convergencePriceOracleContract
        .connect(treasuryDao)
        .setTokenOracleParams(cvx.address, {
          ...
          deltaAggregatorCvgOracle:
            cvx.bond_config.oracleParams.deltaAggregatorCvgOracle, // 10% delta error allowed
          twapInterval: 0,
          maxLastUpdateAggregator: 800_600_400,
        })
    ).wait();
  ...
  if (!h.getTrigger(DEPLOY_BOND_CALCULATOR_CONTRACT)) {
    const bondCalculatorContract = await hre.ethers.deployContract(
    ...
    h.writeFile(
      BOND_CALCULATOR_CONTRACT,
      await bondCalculatorContract.getAddress(),
      DEPLOY_BOND_CALCULATOR_CONTRACT
    );
  }
  ...
  if (!h.getTrigger(CREATE_BOND_FRAX)) {
    ...
    await (
      await iboContract
        .connect(treasuryBonds)
        .createBond(REAL_IBO_PARAMETERS.FRAX.BOND_PARAMETERS)
    ).wait();
  ...
  if (!h.getTrigger(CREATE_BOND_CRV)) {
    ...
    await (
      await iboContract
        .connect(treasuryBonds)
        .createBond(REAL_IBO_PARAMETERS.CRV.BOND_PARAMETERS)
    ).wait();
  ...
  if (!h.getTrigger(CREATE_BOND_CVX)) {
    ...
    await (
      await iboContract
        .connect(treasuryBonds)
        .createBond(REAL_IBO_PARAMETERS.CVX.BOND_PARAMETERS)
    ).wait();
  ...
  if (!h.getTrigger(DEPLOY_PROXY_ADMIN)) {
    const proxyAdmin = await hre.ethers.deployContract(PROXY_ADMIN, [
      treasuryDao,
    ]);
  ...
  if (!h.getTrigger(DEPLOY_CONTROL_TOWER)) {
    ...
    const params = [
      treasuryBonds.address,
      treasuryStaking.address,
      treasuryDao.address,
    ];
    const adminAddress = h.getAddress(PROXY_ADMIN);

    const controlTowerContract = await GlobalHelper.deployProxy(
      sigParams,
      params,
    ...
    h.writeFile(
      CONTROL_TOWER_CONTRACT,
      await controlTowerContract.getAddress(),
  ...
  if (!h.getTrigger(LINK_PRESALE_SEED_CONTRACT)) {
    const seedPresaleContract = await ethers.getContractAt(
  ...
  if (!h.getTrigger(LINK_PRESALE_WL_CONTRACT)) {
    ...
    h.writeFile(
      PRESALE_WL_CONTRACT,
      await wlPresaleContract.getAddress(),
      LINK_PRESALE_WL_CONTRACT
    );
  ...
  if (!h.getTrigger(SET_CVG_VESTING)) {
    ...
    await (
      await vestingContract.connect(treasuryDao).setCvg(h.getAddress("Cvg"))
    ).wait();
  ...
  if (!h.getTrigger(FINISH_PRESALE_ROUND)) {
    ...
    await seedPresaleContract
      .connect(await ethers.getSigner(multisigMainnet))
      .setSaleState(3);
    await hre.network.provider.request({
    ...
    await wlPresaleContract
      .connect(await ethers.getSigner(cvgDeployer))
      .setSaleState(2);
    await hre.network.provider.request({
      method: "hardhat_stopImpersonatingAccount",
      params: [cvgDeployer],
  ...
  if (!h.getTrigger(SEND_PRESALE_NFTS)) {
    ...
    await seedPresaleContract
      .connect(await ethers.getSigner(investorWallet))
      .transferFrom(investorWallet, user2.address, 29);
    //Send WlPresale NFT to user2
    await wlPresaleContract
      .connect(await ethers.getSigner(investorWallet))
      .transferFrom(investorWallet, user2.address, 49);
    ...
    await (
      await vestingContract
        ...
        .createVestingSchedule(
          ethers.parseUnits(
            REAL_VESTING_SCHEDULES.PRESEED_SEED.totalCvgAmount.toString(),
            18
          ),
          (await time.latest()) + 5,
  ...
  if (!h.getTrigger(CREATE_VESTING_SCHEDULE_WL)) {
    ...
    await (
      await vestingContract
        ...
        .createVestingSchedule(
          ethers.parseUnits(
            REAL_VESTING_SCHEDULES.WL.totalCvgAmount.toString(),
            18
          ),
          (await time.latest()) + 5,
  ...
  if (!h.getTrigger(CREATE_VESTING_SCHEDULE_IBO)) {
    ...
    await (
      await vestingContract
        ...
        .createVestingSchedule(
          ethers.parseUnits(
            REAL_VESTING_SCHEDULES.IBO.totalCvgAmount.toString(),
            18
          ),
          (await time.latest()) + 5,
  ...
  if (!h.getTrigger(CREATE_VESTING_SCHEDULE_TEAM)) {
    ...
    await (
      await vestingContract
        ...
        .createVestingSchedule(
          ethers.parseUnits(
            REAL_VESTING_SCHEDULES.TEAM.totalCvgAmount.toString(),
            18
          ),
          (await time.latest()) + 5,
  ...
  if (!h.getTrigger(CREATE_VESTING_SCHEDULE_DAO)) {
    ...
    await (
      await vestingContract
        ...
        .createVestingSchedule(
          ethers.parseUnits(
            REAL_VESTING_SCHEDULES.DAO.totalCvgAmount.toString(),
            18
          ),
          (await time.latest()) + 5,
