...
describe("Oracle Testing", () => {
    const deltaPercentage = 0.02; // 2% error marging with coingecko API
    const UNIV3 = 0;
    const UNIV2 = 1;
    ...
    before(async () => {
        ...
        prices = await ApiHelper.getCoinGeckoTokenPrices(
            ["ethereum", "curve-dao-token", "convex-finance", "stake-dao", "conic-finance", "tokemak", "frax-share"],
            "usd"
        );
    ...
    it("Should compute Token prices in $ ", async () => {
        for (const token of tokens) {
            const priceInPool = (await convergencePriceOracleContract.getPriceOracle(token.token))[0];
